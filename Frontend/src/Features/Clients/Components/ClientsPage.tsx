    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const resultCount = await clientService.getCount(filterQuery);

                setClientsCount(resultCount);
                const result = await clientService.getAll(filterQuery, "", currentPage, pageSize);
                setClients(result || []);
                    ...
                    <div className="filters-container">
                        {clients.length > 0 && (
                            <h2 className="font-semibold text-xl mb-4">
                                {`${t('clients.foundCount')} ${clientsCount} `}
                            </h2>
                        )}
